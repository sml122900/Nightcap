...
- ~~[높음] Android 15 limited 접근에서 자동수집이 조용히 무력화~~ **해소(2026-08-10)** — `MediaAccess`(`all`/`limited`/`none`) 단일 타입으로 판별을 통일하고(`80792ef`), limited일 때 설정·온보딩에 안내 + "전체 허용" 액션을 띄운다(`2856d3a`, `f332a60`). limited는 권한이 있는 상태라 토글을 막지 않는다. **다만 "전체 허용" 액션이 실사용에서 선택기를 실제로 여는지는 미검증** — `presentPermissionsPicker`는 "사용자가 원래 limited를 허용한" 경우에만 뜨고 그 외엔 no-op인데, 테스트의 limited는 `pm grant`로 만든 것이라 그 조건을 못 탔다. 손으로 한 번 확인 필요(`docs/decisions/android15-limited-media-access.md` 말미)
...
- 사진 권한은 2상태가 아니라 3상태다 — `READ_MEDIA_IMAGES.granted` 단독으로 판별하면 **limited를 denied로 오독한다**(limited에선 `READ_MEDIA_VISUAL_USER_SELECTED`만 부여되고 `READ_MEDIA_IMAGES`는 false다). 항상 `screenshotScan`의 `MediaAccess`(`all`/`limited`/`none`)를 쓸 것. 그리고 limited에서는 **새로 찍은 스크린샷이 스캔에 절대 안 잡힌다**(사용자가 고른 자산 집합만 보이므로) — 실측 근거는 `docs/decisions/android15-limited-media-access.md`.
- 권한 상태를 손으로 재현할 때: 이미 `USER_SELECTED`가 부여된 앱은 `requestPermissionsAsync`가 클래식 다이얼로그 대신 **사진 선택기**를 띄우고, 취소해도 limited가 유지된다. 반면 권한 이력이 없는 깨끗한 상태에서는 클래식 다이얼로그가 뜨고 취소하면 `none`이다. 테스트 전 `adb shell pm revoke`로 두 권한을 **모두** 회수하거나 재설치해서 플래그(`USER_SET`/`USER_FIXED`)까지 비울 것.
- `MediaLibrary.presentPermissionsPicker()`는 **"사용자가 원래 limited를 허용한" 경우에만** 모달을 띄우고 그 외엔 조용한 no-op이다(SDK 문서 명시). `pm grant`로 만든 limited에서는 안 열린다 — 이 API에 의존하는 UI는 실사용 경로로 한 번 확인해야 한다.
