...
- **디자인 시스템 + 라이트 모드** — 스펙은 PROJECT.md §5(디자인 시스템) / §4(화면별 테마 배정), 설계 근거는 `docs/decisions/theme-surface-model.md`. 기능 변경 0의 순수 리페인트.
  ...
  - **`bg` 값 확정(2026-08-11)**: `#0B0D12` 유지, 순수 `#000000`으로 안 바꾼다. 미결 항목 종결 — 이후 별도 지시 없이 이 값을 바꾸지 않는다.
...
- `MediaLibrary.presentPermissionsPicker()`는 Android에서 **별도의 "사진 재선택" 모달이 아니라 그냥 권한 재요청**이다(`SystemPermissionsDelegate.kt`가 `requestPermissions`를 그대로 호출). 사용자가 한 번 선택을 내린 뒤(`USER_SET`)에는 OS가 아무것도 안 띄우므로 **조용한 no-op**이 되고, 그건 도움이 필요한 바로 그 상태다. limited를 넓히는 UI는 `Linking.openSettings()`로 보낼 것 — 항상 열린다. `pm grant`로 만든 limited에서는 특히 안 열리므로 이 API를 테스트로 검증했다고 믿지 말 것.
