...
- **[높음] 자동수집 ON + 사진 권한 없음 → 정리 모드 전체 차단.** `TriageScreen:329`가 스캔 권한으로 덱 전체를 막는데, 공유시트로 담은 카드는 그 권한이 필요 없다. 게다가 온보딩이 이 상태를 직접 만든다 — `OnboardingScreen.handleAutoScanToggle`이 `requestMediaAccess()` 결과와 무관하게 토글을 ON으로 저장하므로, "켜볼까?" → 권한 거부 → 코어 루프 사망. W3-1(스캔이 유일 유입) 시절 가드가 W3-2(공유시트 주 경로) 전환 때 안 고쳐진 것. 재현·확인 완료
