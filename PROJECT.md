...
**사진 권한 3상태 동작** (Android 14+, `services/screenshotScan.ts`의 `MediaAccess`)

| 상태 | 토글 | 스캔 | 안내 |
|---|---|---|---|
| `all` | 저장값 유지 | 정상 | 없음 |
| `limited` | **ON 유지** | 사용자가 고른 자산만 — **새로 찍은 스크린샷은 절대 안 잡힌다** | 설정·온보딩·정리 화면에 "일부 사진만 접근 중" + "전체 허용" |
| `none` | **OFF로 정정 후 저장** | 조용히 no-op | 없음 |

- `limited`는 거부가 아니다. 실제로 접근 권한이 있고 고른 사진은 담기므로 토글을 막지 않는다. 대신 안 되는 것을 명시하고 확장 경로를 준다. 토글이 꺼져 있으면 안내도 띄우지 않는다(사용자가 원한 상태라 노이즈)
- 판별은 `READ_MEDIA_IMAGES.granted`가 아니라 `accessPrivileges` 기준이다. limited에서 그 권한은 false라 단독 판별하면 거부로 오독한다
- 정리 모드 진입은 사진 권한과 **무관**하다. 공유시트로 담은 카드는 그 권한이 필요 없다(§3.2)
- 배경·실측은 `docs/decisions/android15-limited-media-access.md`
...
- **다중 선택 삭제**: 카드 길게 누르기 → 선택 모드 진입(햅틱 1회) + 해당 카드 체크. 이후 탭은
  상세 진입 대신 선택 토글, 마지막 1개 해제 시 모드 자동 종료. 하단 액션바("N개 선택" + 삭제)가
  평소의 "휴지통 N" 진입 버튼 자리를 대신한다. 삭제는 확인 다이얼로그("N개를 휴지통으로 이동") 뒤
  기존 소프트 삭제 경로 그대로(7일 보관, 영구 삭제 아님). 선택 상태는 화면 이탈 시 소멸, 별점
  필터를 바꿔도 유지되며(화면 밖으로 나간 항목도 액션바 카운트에 포함) 전체 선택 버튼은 없다 —
  아카이브 앱에서 "전체 삭제"는 사고 경로라 의도적으로 안 둠. 뒤로가기는 선택 모드만 닫는다.
- **상세 화면 좌우 스와이프**: 탭한 카드에서 진입해 보관함의 **현재 필터·정렬 결과와 동일한
  순서**로 인접 카드를 넘나든다(별점 필터를 걸고 들어왔으면 그 목록만 순회). 첫/마지막 카드에서는
  바운스, 순환 없음. 가로 페이징 FlatList로 구현 — 페이지 내부 세로 스크롤(메타 편집 영역)과
  방향이 직교라 커스텀 제스처 중재가 필요 없다. 인접 카드는 처음부터 마운트돼 스와이프 직후
  빈 화면이 뜨지 않는다. 탐색 중 삭제한 카드는 목록에서 바로 빠지고 다음 카드로 자연스럽게
  이어진다(마지막 카드를 지운 경우만 스크롤 위치를 명시적으로 보정).
