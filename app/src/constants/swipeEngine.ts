...
/** upness distance (px) that maps to a 5.0 prefill rating */
export const RATE_SPAN = 230;
/** rotate(deg) = dx * ROT */
...
/** live drag rating preview: clamp(round(upness / RATE_SPAN * 5 * 2) / 2, 0.5, 5) */
export function prefillFromUpness(upness: number): number {
  'worklet';
  return Math.min(5, Math.max(0.5, Math.round((upness / RATE_SPAN) * 5 * 2) / 2));
}

/** star-bar drag: same 0.5-unit rounding, but 0 is a valid ratio endpoint (min visual value 0.5) */
...
/** rate mode docking: top card animates to translateY(-152) scale(.9), 0.3s cubic-bezier(.2,.8,.2,1) */
export const RATE_DOCK = { y: -152, scale: 0.9 } as const;
export const RATE_DOCK_DURATION = 300;
export const RATE_DOCK_EASING = Easing.bezier(0.2, 0.8, 0.2, 1);
