...
import { RateModeLayer } from '../triage/RateModeLayer';
import { Capture } from '../../types/capture';

interface DetailPageProps {
  item: Capture & { stars: number };
  width: number;
  /** Tells the pager to drop this card from the list and settle on a neighbor. */
  onDeleted: () => void;
}

/**
 * One page of the LibraryDetail pager (지시서 B) — the per-item body that used to be the whole
 * of LibraryDetailScreen before it became a horizontal pager. Owns its own edit-draft state
 * (title/stars/link) keyed to this item's id via closures, so paging between cards can't cross-wire
 * an edit onto the wrong one — there's no shared "current index" driving these fields.
 */
export function DetailPage({ item, width, onDeleted }: DetailPageProps) {
      ...
      <ScrollView contentContainerStyle={[styles.body, { paddingBottom: insets.bottom + 24 }]}>
        {item.kind !== 'drm' && item.imageUri ? (
          <Image
            ...
            onLoad={(e) => {
              const { width: w, height } = e.nativeEvent.source;
              if (w && height) setAspectRatio(w / height);
            }}
          />
        ) : (
